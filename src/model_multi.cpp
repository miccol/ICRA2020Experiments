...
void ModelMulti::set_current_physical_state(int current_physical_state)
{
    current_physical_state_ = current_physical_state;
}

ModelMulti::ModelMulti(int physical_initial_state, std::vector<Action> actuations, std::vector<Action> sensing,
...
std::vector<bool> ModelMulti::take_action(Action action, WorkspaceMulti *ws)
{
...
bstate ModelMulti::updateBelief(bstate current_belief, Action action, std::vector<bool> observation, bool *flag)
{
...
int ModelMulti::current_physical_state()
{
    return current_physical_state_;
}

void ModelMulti::get_next_belief_states(bstate belief_state, std::vector<bstate> *next_belief_states,
                                   std::vector<Action> *actions_list, std::vector<std::vector<bool>> *observations_list, std::vector<float> *entropies_list, std::vector<float> *prob_list )
{
    ...
    //     if (entropy(belief_state) > 0.0)
    {
        for (auto it_s = sensing_.begin(); it_s != sensing_.end(); it_s++)
        {
            ...
            std::vector<std::vector<bool>> obs = observation_map_[*it_s];
            ...
            for (auto it_ol = obs.begin(); it_ol != obs.end(); it_ol++)
            {
                ...
                for (auto it_o = (*it_ol).begin(); it_o != (*it_ol).end(); it_o++)
                {
                    ...
                    //                     next_belief_state[p_state_from] = 0.0;
                    //                 }
